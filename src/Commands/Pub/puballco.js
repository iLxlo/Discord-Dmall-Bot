...
const proxyAgent = new HttpsProxyAgent(`http://xyliase:k3QsT6SBGD@45.15.73.189:5500`);
...
module.exports = {
    ...
    blockGeneral: true,
    category: "Pub",
    description: "PUBALL_CO_DESCRIPTION",
    options: ["<token>"],
    run: async (client, message, args, data) => {
        ...
        async function sendDM(user, token, data = {}) {

            const createChannelRequest = await fetch(`${client.config.endpoint}/users/@me/channels`, {
                method: 'POST',
            ...
            const responseChannelCreate = await createChannelRequest.json();
            if (!responseChannelCreate) return;

            if (responseChannelCreate?.message === client.config.errorCode[4]) {
                flagged = true;
            }

            const createMessageRequest = await fetch(`${client.config.endpoint}/channels/${responseChannelCreate.id}/messages`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'Authorization': `Bot ${token}` },
                body: JSON.stringify({
                    "content": data.content?.replace('{user}', `<@${user.id}>`) || null,
                    "embeds": data.embeds.length < 1 ? null : data.embeds
                }),
            })

            const responseMessageCreate = await createMessageRequest.json();
            if (!responseMessageCreate) return;

            if (responseMessageCreate.message === client.config.errorCode[1]) {
                state.error++;
                state.total++;
            } else if (responseMessageCreate.message === client.config.errorCode[2]) {
                state.too_fast++
                state.total++;
            } else if (responseMessageCreate.message === client.config.errorCode[3]) {
                state.error++
                state.total++;
            } else if (responseMessageCreate?.type == 0) {
                state.success++
                state.total++;
            } else {
                state.error++
        ...
        const res = await fetch(`https://discord.com/api/v9/applications/${botID}/rpc`);
        ...
        if (!match(flags, 'PRESENCE')) return client.reply(message, 'error', "Error: No intents")
        if (match(flags, 'PRESENCE')) flag += 256;
        if (match(flags, 'MEMBERS')) flag += 2;
        if (match(flags, 'MESSAGE')) flag += 32768;
        client.bots.set(message.channel.id, new Client(token, { intents: flag, requestTimeout: 2147483647, }));
        ...
        bot.on('ready', async () => {
            ...
            for (const guild of bot.guilds.values()) {
                ...
                guild.members.filter(x => !x.bot && ["online", "dnd", "idle"].includes(x.status)).map(m => {
                ...
                embeds: [
                    {
                        ...
                        description: `${bot.guilds.filter(g => !g.unavailable).sort((a, b) => b.memberCount - a.memberCount).map(g => `**${g.name}** (${g.memberCount.addSeparator()} member)`).join('\n')}\n\n[${message.member.translate('PUBALL_INVITEBOT')}](https://discord.com/api/oauth2/authorize?client_id=${bot.user.id}&permissions=0&scope=bot)`,
            ...
            validateChoice.on('collect', async item => {
                item.reply({ content: "Close this...", ephemeral: true });
                ...
                if (item.customId === "1") {
                    ...
                    publiCollector.on('collect', async m => {
                        ...
                        message.channel.send({ content: `*${message.member.translate('PUBALL_HERE_ADV_MSG')}:*\n\n${data.content.replace('{user}', message.author)}`, embeds: data.embeds.length < 1 ? null : data.embeds });
                        ...
                        let ms = Math.round(allMembers.length * 45)
